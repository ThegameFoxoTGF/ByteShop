...
function CategoryListPage() {
    ...
    const handleDelete = async (id) => {
        if (window.confirm('คุณแน่ใจว่าต้องการลบหมวดหมู่นี้ใช่หรือไม่?')) {
            try {
                ...
                const message = error.response?.data?.message || 'ลบหมวดหมู่ไม่สำเร็จ';
                toast.error(message);
            }
        }
    };
    ...
    return (
        <div className="p-6 max-w-7xl mx-auto space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-sea-text">Category Management</h1>
                    <p className="text-sea-subtext mt-1">จัดการรายชื่อหมวดหมู่ในระบบ</p>
                </div>
                <Link
                ...
                {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-20 text-center">
                        <div className="w-20 h-20 bg-slate-50 rounded-full flex items-center justify-center mb-4">
                            <Icon icon="ic:round-category" className="text-slate-300 w-10 h-10" />
                        </div>
                        <h3 className="text-lg font-semibold text-sea-text">ไม่พบข้อมูลหมวดหมู่</h3>
                        <p className="text-sea-subtext mt-1">ลองเปลี่ยนคำค้นหา หรือเพิ่มหมวดหมู่ใหม่</p>
                    </div>
                ) : (
                    <>
                            ...
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="bg-slate-50 border-b border-slate-100 text-xs uppercase tracking-wider text-sea-muted font-semibold">
                                        <th className="px-6 py-4 rounded-tl-2xl">ชื่อหมวดหมู่</th>
                                        <th className="px-6 py-4">ป้ายกำกับ</th>
                                    ...
                                    {categories.map((category) => (
                                        ...
                                        >
                                            <td className="px-6 py-4">
                                                <span className="font-medium text-sea-text">{category.name}</span>
                                            </td>
                                            <td className="px-6 py-4 text-sea-text">
                                                {category.label || '-'}
                                            </td>
                                            <td className="px-6 py-4 text-sea-muted text-sm font-mono">
                                                {category.slug}
                                            </td>
